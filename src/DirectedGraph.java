...
public class DirectedGraph
{
    ...
    {  //have an uphill adjacency list and a downhill adjacency list
        nodes = new ArrayList<DirectedGraphNode>();
    ...
    {
        /*
            Adjacency List format: this array list will hold linked list
            that have all the vertices that a node connects to. The size
            of this table corresponds to the number of nodes in the tree
         */
        ...
        // base case: checks if there's any children
        if(root.outgoingNodes.size() == 0)
        {
            return 1;
        }

        boolean minCover = coverage(root)[0];
        if (!minCover)
        {
            cover++;
        }
    ...
    int cover = 0;
    private boolean[] coverage(DirectedGraphNode root) {
        boolean[] cov = new boolean[2];
        // base case: current node is covered and parent is not covered
        if(root.outgoingNodes.size() == 0)
        {
            cov[0] = true;
            cov[1] = false;
        ...
        boolean[] left = coverage(root.outgoingNodes.getFirst());
        boolean[] right = coverage(root.outgoingNodes.getLast());

        // if child node is not covered
        if(!left[0] || !right[0])
        {
            cover++;
            cov[0] = true;
        ...
        // either node is covered or parent is covered
        return new boolean[] {left[1] || right[1]};
    }
    ...
    {
        // TODO: ADD MORE DATAFIELDS FOR THE NODE
        private int data;
        ...
        {
            // information stored in this node
            protected E data;
